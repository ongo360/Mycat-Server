...
public class MySqlOutputVisitor extends SQLASTOutputVisitor implements MySqlASTVisitor {
    ...
    public boolean visit(MySqlTop x) {
        print0(ucase ? "TOP " : "top ");

        boolean paren = false;

        if (x.getParent() instanceof MySqlUpdateStatement || x.getParent() instanceof MySqlInsertStatement) {
            paren = true;
            print('(');
        }
    ...
    public void endVisit(MySqlTop x) {
